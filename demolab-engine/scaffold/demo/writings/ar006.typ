...
#let body = [
  ...
  1952, with state $bold(x) = (V, m, h, n)^top$: one voltage and three gating variables.
  Everything below is that one system, taken apart one relation at a time.

  == Current balance

  The membrane is a capacitor in parallel with three voltage-gated conductances. Charge
  ...
  clamp data, no first-principles derivation behind them. They are what turns the abstract
  ...
  anyway: the singularity is removable, and the value is just the limit:
  ...
  slope factor $k$ than to go through the rate functions. Its derivative has the tidy
  ...
  poised to spike becomes a matter of eigenvalues. Write the system as
  ...
  spikes open conductances that pull the membrane toward a synaptic reversal potential. A spike
